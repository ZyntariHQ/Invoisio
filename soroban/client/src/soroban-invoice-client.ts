...
export class SorobanInvoiceClient {
   ...
   */
  async migrateLegacyPayments(invoiceIds: string[]): Promise<TransactionResult> {
    this.requireSigner();
    if (invoiceIds.length > MAX_LEGACY_MIGRATION_BATCH) {
      throw new Error(
        `invoiceIds must have at most ${MAX_LEGACY_MIGRATION_BATCH} entries, got ${invoiceIds.length}`,
      );
    }
    const account = await this.server.getAccount(this.keypair!.publicKey());
    const caller = this.keypair!.publicKey();
